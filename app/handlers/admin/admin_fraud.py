...
async def fraud_queue_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        with FraudDetector() as detector:
            ...
            if not pending:
                await update.message.reply_text(
                    ...
                    "No pending reviews! All clear. ðŸŽ‰",
                    parse_mode="Markdown"
                )
                return
            
            # Build queue message
            message = "ðŸ›¡ï¸ **FRAUD REVIEW QUEUE**\n\n"
            message += f"**Total: {len(pending)} pending reviews**\n\n"
            
            for idx, item in enumerate(pending[:10], 1):  # Show first 10
                emoji = "ðŸš¨" if item['review_status'] == 'HIGH_RISK' else "âš ï¸"
        ...
        await update.message.reply_text(
            f"âŒ Error getting fraud queue: {str(e)}"
        )
...
async def fraud_review_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("âŒ Invalid referral ID. Must be a number.")
        return
    ...
    try:
        ...
        if not referral:
            await update.message.reply_text(f"âŒ Referral {referral_id} not found.")
            session.close()
            return
        ...
        emoji = "ðŸš¨" if referral.review_status == 'HIGH_RISK' else "âš ï¸"
    ...
    try:
        with FraudDetector() as detector:
            ...
            if success:
                ...
                await update.message.reply_text(
                    f"âŒ Failed to approve referral #{referral_id}.\n"
    ...
    try:
        with FraudDetector() as detector:
            ...
            if success:
                ...
                await update.message.reply_text(
                    f"âŒ Failed to reject referral #{referral_id}.\n"
                    f"It may not exist or already be processed."
                )
...
async def fraud_stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        with FraudDetector() as detector:
            ...
            message = (
                "ðŸ“Š **FRAUD DETECTION STATISTICS**\n\n"
                ...
                f"ðŸš¨ High risk: {stats['high_risk']}\n"
                f"â›” Rejected: {stats['rejected']}\n\n"
                f"â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”\n"
                f"**Approval Rate:** {stats['approval_rate']}%\n\n"
                f"ðŸ’¡ Use `/fraud_queue` to review pending cases"
            )
